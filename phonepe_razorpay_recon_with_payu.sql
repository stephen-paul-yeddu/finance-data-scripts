...
(select a.created_at,a.entity_id,a.type,a.transaction_id,a.bbps_ref_id,a.credit as amount,
a.product_fallback,
a.product,
case when b.transaction_id is not null then True else False end as refunded,
b.debit as refund_amount,
...
where a.type in ('payment','reversal')
and a.product = 'BBPS'
...
case when b.forward_merchant_transaction_id is not null then True else False end as refunded,
...
from
(select 'Razorpay' as gateway,created_at,'' as merchant_id,entity_id as merchant_reference_id,amount,refunded,refund_amount,
bbps_ref_id,payu_amount_paid,payu_amount_refund
from razorpay_final_data

union all

select 'Phonepe' as gateway,date(transaction_date) as transaction_date,merchant_id,merchant_reference_id,total_transaction_amount,refunded,total_refund_amount,
bbps_ref_id,payu_amount_paid,payu_amount_refund
from phonepe_final_data
)