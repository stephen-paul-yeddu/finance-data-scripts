...
SELECT
coalesce(a.state,b.state) as state,coalesce(a.product,b.product) as product,
a.paid_amount,
b.refund_amount
FROM
(SELECT state,product,sum(amount) as paid_amount from data
where type in ('payment','reversal')
group by 1,2) a
FULL OUTER JOIN
(SELECT state,product,sum(amount) as refund_amount from data
where type in ('refund')
group by 1,2) b on a.state = b.state and a.product = b.product